...
"Inicializa o Contador e Variável que guarda a última empresa.
lv_count = 0.
lv_last_bukrs = ''.
...
*  do Razão e Texto das contas do Razão.
  ...
  SELECT saknr,    "Seleciona o Nº conta do Razão
         ...
         AND spras = 'P'.  "Onde o idioma é PT
...
SORT t_output BY bukrs. "Ordena a Tabela de Saída por empresa.

  "Impressão dos registros por empresa.
  LOOP AT t_output INTO ls_output.
    IF lv_last_bukrs <> ls_output-bukrs.
      IF lv_last_bukrs <> ''.
        WRITE: / 'Quantidade de Contas Vinculadas:', lv_count.
        WRITE: / '-----------------------------------------------'.
      ENDIF.
    ENDIF.
    IF lv_last_bukrs <> ls_output-bukrs.
    ...
    ENDIF.
    WRITE: / '-----------------------------------------------'.
    READ TABLE t_skb1 INTO ls_skb1 WITH KEY saknr = ls_output-saknr.
      WRITE: / 'Nº conta do Razão:', ls_skb1-saknr,
             / 'Data de Criação do Registro:', ls_output-erdat.
      READ TABLE t_skat INTO ls_skat WITH KEY saknr = ls_output-saknr.
        WRITE: / 'Texto das Contas do Razão', ls_skat-txt20.
        lv_last_bukrs = ls_output-bukrs.
        lv_count = lv_count + 1.
        WRITE: / '-----------------------------------------------'.
  ENDLOOP.

  "Para pegar o último registro.
  WRITE: / 'Quantidade de Contas Vinculadas:', lv_count.
  WRITE: / '-----------------------------------------------'.
